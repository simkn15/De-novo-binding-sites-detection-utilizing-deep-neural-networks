...
from keras.layers import Input, Dense, Lambda, Flatten, Reshape, Conv1D, Conv2D, Multiply
...
conv = Conv2D(3, kernel_size=(6, 30), use_bias=False,
             padding='valid', activation='relu', strides=(1,3))(inputRes)
maxLambda = Lambda(wta_last_ax)(conv) # TODO Change this to maxPooling as we are using images ??? No longer just a 1-dim sequence
...
if __name__ == "__main__":
    ...
    attentionConv = Model(input, conv)
    profilesConv = attentionConv.predict(data.xTestImages, batch_size=batch_size)
    print("profilesConv shape:", profilesConv.shape)
